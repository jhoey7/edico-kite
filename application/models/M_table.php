...
class M_table extends CI_Model {
	...
	function supplier($act="") {
		$header = "Data Referensi Supplier";
		$header_content = "Menampilkan seluruh data Supplier yang terdapat di Supplier Satuan.";
		if (grant() == "W") {
			$checked = true;
		} else {
			$checked = false;
		...
		$this->load->library('newtable');
		$SQL = "SELECT a.kd_supplier, a.nm_supplier as 'NAMA SUPPLIER', a.almt_supplier as 'ALAMAT SUPPLIER', 
				b.uraian_negara as 'NEGARA SUPPLIER', a.npwp_supplier as 'NPWP' 
	...
	function stockopname($act="") {
		...
		$SQL = "SELECT DATE_FORMAT(tgl_stock, '%d %M %Y %H:%i') AS 'TGL. STOCKOPNAME', COUNT(id) AS 'JML. DETIL BARANG', DATE_FORMAT(tgl_stock, '%Y%m%d%H%i') as id
				FROM tm_stockopname 
				WHERE kode_trader = ".$this->db->escape($kode_trader)." GROUP BY DATE_FORMAT(tgl_stock, '%Y%m%d%H%i')";
		$proses = array(
